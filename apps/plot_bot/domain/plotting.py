...
class PlottingBotPlotting:
    ...
    def plot_double(self):
        ...
        if np.mean(np.abs(np.diff(self.current_scan.delays)))  < SPECTROGRAM_THRESHOLD:
    ...
    def plot_double(self):
        use_spec = np.mean(np.abs(np.diff(self.current_scan.delays))) < SPECTROGRAM_THRESHOLD
