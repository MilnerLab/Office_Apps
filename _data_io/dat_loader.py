...
def load_time_scan(path: Path, config: IonDataAnalysisConfig | None = None) -> C2TScanData:
    ...
    return C2TScanData(delays=delays, measured_values=c2ts, run_id=None, ions_per_frame=ions, config=config, file_path=path)


def load_time_scans(paths: list[Path], config: IonDataAnalysisConfig | None = None) -> list[C2TScanData]:
